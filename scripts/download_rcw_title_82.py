#!/usr/bin/env python3
"""
Download Revised Code of Washington (RCW) Title 82 - Revenue

Scrapes https://app.leg.wa.gov/rcw/default.aspx?Cite=82 and downloads all sections
from priority chapters (82.04, 82.08, 82.12, etc.) related to sales/use tax.

Usage:
    python scripts/download_rcw_title_82.py
    python scripts/download_rcw_title_82.py --chapters 82.04,82.08,82.12
    python scripts/download_rcw_title_82.py --limit 10  # For testing
    python scripts/download_rcw_title_82.py --format html  # HTML only
    python scripts/download_rcw_title_82.py --format pdf   # PDF only
"""
...
class RCWTitleScraper:
    ...
    def scrape_chapters(self, title_cite: str = "82") -> List[Dict]:
        """
        Scrape title page to get all chapters

        Args:
            title_cite: Title number (default: "82")
    ...
    def scrape_chapter_sections(self, chapter_cite: str) -> List[Dict]:
        ...
        Args:
            chapter_cite: Chapter citation (e.g., "82.04")
    ...
    def download_section(self, section: Dict, format: str, chapter_dir: Path) -> bool:
        """
        Download a single RCW section in specified format(s)
    ...
    def download_all(
        ...
        Args:
            chapter_filter: List of chapter cites to download (e.g., ['82.04', '82.08']). None = all
            format: 'html', 'pd', or 'both'
...
def main():
    ...
    parser.add_argument(
        "--chapters",
        help=f"Comma-separated chapters to download (e.g., '82.04,82.08,82.12'). Default: {
    ...
    # Initialize scraper
    scraper = RCWTitleScraper(output_dir=args.output_dir, rate_limit=args.rate_limit)
