...
class Phase2PatternLearner:
    ...
    def get_top_indicators(self, top_n=20):
        ...
        for i, (kw, count) in enumerate(sorted(
        ...
        for i, (kw, count) in enumerate(sorted(
            self.patterns['pass_keywords'].items(),
            key=lambda x: x[1],
            reverse=True
        )[:top_n], 1):
            print(f"  {i:2}. '{kw}': {count} occurrences")
        ...
        for product_type, distribution in self.patterns['product_type_patterns'].items():
            total = sum(distribution.values())
            if total >= 10:  # Minimum sample size
                refund_count = sum(
                    count for decision, count in distribution.items()
        ...
        for i, (ptype, rate, count) in enumerate(
            sorted(product_refund_rates, key=lambda x: (x[1], x[2]), reverse=True)[:top_n], 1
        ):
        ...
        for i, (tax_cat, rate, count) in enumerate(
            sorted(tax_refund_rates, key=lambda x: (x[1], x[2]), reverse=True)[:top_n], 1
        ):
...
def main():
    ...
    print("You can now use these learned patterns to classify new invoices automatically.")
