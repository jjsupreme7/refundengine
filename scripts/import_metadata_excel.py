...
class ExcelMetadataImporter:
    ...
    def fetch_current_data(self, table_name: str, ids: List[str]) -> Dict[str, Dict]:
        ...
        for i in range(0, len(ids), batch_size):
            ...
            result = self.supabase.table(table_name).select('*').in_('id', batch_ids).execute()
    ...
    def detect_changes(self, excel_rows: List[Dict], current_data: Dict[str, Dict], table_name: str) -> List[Dict]:
        ...
        for excel_row in excel_rows:
            ...
            for field, excel_value in excel_row.items():
                ...
                if field in ['id', 'document_id', 'created_at', 'updated_at', 'embedding', 'chunk_text']:
                    continue
    ...
    def show_diff(self, changes: List[Dict]):
        ...
        for i, change in enumerate(changes, 1):
            ...
            display_name = (excel_row.get('title') or
                           excel_row.get('citation') or
                           excel_row.get('vendor_name') or
                           record_id[:8])
    ...
    def run(self, auto_confirm: bool = False):
        ...
        try:
            ...
            if docs_rows:
                ...
                if ids:
                    ...
                    changes = self.detect_changes(docs_rows, current_data, 'knowledge_documents')
        ...
        try:
            ...
            if chunks_rows:
                ...
                if ids:
                    ...
                    changes = self.detect_changes(chunks_rows, current_data, 'tax_law_chunks')
                    all_changes.extend(changes)
        except Exception as e:
        ...
        try:
            ...
            if vendor_rows:
                ...
                if ids:
                    current_data = self.fetch_current_data('vendor_background_chunks', ids)
                    changes = self.detect_changes(vendor_rows, current_data, 'vendor_background_chunks')
                    all_changes.extend(changes)
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Import metadata changes from Excel to Supabase')
    parser.add_argument('--file', type=str, required=True,
                       help='Path to edited Excel file')
