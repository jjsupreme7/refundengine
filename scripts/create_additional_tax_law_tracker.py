...
def classify_document(filename: str) -> dict:
    ...
    # ESSB 5814 Detection
        ...
        elif 'it services' in filename_lower or 'information technology' in filename_lower:
    ...
    # Industry Guides
        ...
        industry = filename.replace('_tax_guide.pdf', '').replace('.pdf', '').replace('_', ' ')
        classification.update({
...
def main():
    ...
    if os.path.exists(essb_kb_path):
        ...
        for filename in sorted(essb_files):
            ...
            doc["file_path"] = f"knowledge_base/states/washington/essb_5814_oct_2025/{filename}"
            doc["status"] = "Ready to Ingest" if not filename.startswith("0") else "Already Ingested"
    ...
    if os.path.exists(additional_tax_law_path):
        additional_files = [f for f in os.listdir(additional_tax_law_path) if f.endswith('.pdf')]
        ...
        for filename in sorted(additional_files):
            ...
            if doc["document_type"] == "ETA":
                ...
                doc["file_path"] = f"knowledge_base/states/washington/other_guidance/{filename}"
    ...
    with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
        ...
        other_df = df[~df['document_type'].isin(['ETA', 'Tax Type Guide', 'Industry Guide']) &
