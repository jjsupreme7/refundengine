...
from core.chunking_with_pages import extract_text_with_page_mapping, find_chunk_page_numbers, format_section_with_page
...
def add_page_numbers():
    ...
    for doc in documents:
        ...
        try:
            full_text, page_map, total_pages = extract_text_with_page_mapping(source_file)
        ...
        chunks_result = supabase.table('tax_law_chunks').select('*').eq('document_id', doc_id).order('chunk_number').execute()
        ...
        for chunk in chunks:
            ...
            try:
                ...
                if updated_count % 10 == 0:
                    print(f"  Updated {updated_count}/{len(chunks)} chunks...", end='\r')
...
def verify_page_numbers():
    ...
    # Sample a few chunks
    chunks = supabase.table('tax_law_chunks').select('citation, chunk_number, section_title').limit(5).execute()
