...
class HistoricalKnowledgeImporter:
    ...
    def __init__(self, dry_run: bool = False):
        ...
        # Extracted patterns
        self.vendor_patterns = defaultdict(lambda: {
            'refund_count': 0,
            'no_opp_count': 0,
        ...
        self.category_patterns = defaultdict(lambda: {
            'refund_count': 0,
            'no_opp_count': 0,
            'refund_bases': Counter(),
        ...
        self.citation_patterns = defaultdict(lambda: {
            'usage_count': 0,
            'example_cases': [],
        })

        self.keyword_patterns = defaultdict(lambda: {
            'refund_count': 0,
            'no_opp_count': 0,
            'refund_bases': Counter(),
            'keywords': set(),
    ...
    def import_from_excel(self, excel_path: str):
        ...
        logger.info(f"Filtered to {len(analyzed_df)} analyzed records (ignoring {len(df) - len(analyzed_df)} unanalyzed)")
    ...
    def _filter_analyzed_records(self, df: pd.DataFrame, column_map: Dict) -> pd.DataFrame:
        """
    ...
    def _extract_description_keywords(self, description: str) -> List[str]:
        ...
        stopwords = {'the', 'a', 'an', 'and', 'or', 'but', 'for', 'of', 'to', 'in', 'on', 'at', 'by', 'from', 'with', 'is', 'was', 'are', 'were'}
    ...
    def _extract_vendor_patterns(self, df: pd.DataFrame, column_map: Dict):
        ...
        product_col = (column_map.get('vertex_category') or
                      column_map.get('tax_category') or
                      column_map.get('material_group') or
            ...
            if tax_amount_col and pd.notna(row.get(tax_amount_col)):
                try:
                    ...
                    if len(self.vendor_patterns[vendor]['sample_tax_amounts']) < 10:
                        self.vendor_patterns[vendor]['sample_tax_amounts'].append(tax_amount)
                except:
                    pass
    ...
    def _extract_category_patterns(self, df: pd.DataFrame, column_map: Dict):
        ...
        for category_type, category_col in category_columns:
            for idx, row in df.iterrows():
                ...
                if refund_basis_col and pd.notna(row.get(refund_basis_col)) and is_refund:
                    refund_basis = str(row.get(refund_basis_col)).strip()
                    self.category_patterns[pattern_key]['refund_bases'][refund_basis] += 1
    ...
    def _extract_citation_patterns(self, df: pd.DataFrame, column_map: Dict):
        ...
        for idx, row in df.iterrows():
            ...
            if len(self.citation_patterns[refund_basis]['example_cases']) < 3:
                ...
                self.citation_patterns[refund_basis]['example_cases'].append(str(vendor))
    ...
    def _extract_keyword_patterns(self, df: pd.DataFrame, column_map: Dict):
        ...
        for idx, row in df.iterrows():
            ...
            if refund_basis_col and pd.notna(row.get(refund_basis_col)) and is_refund:
                refund_basis = str(row.get(refund_basis_col)).strip()
                self.keyword_patterns[keyword_signature]['refund_bases'][refund_basis] += 1
    ...
    def _import_vendor_patterns(self):
        ...
        for vendor, data in self.vendor_patterns.items():
            ...
            vendor_keywords = list(data['vendor_keywords']) if data['vendor_keywords'] else None
            ...
            try:
                ...
                existing = self.supabase.table('vendor_products').select('*').eq('vendor_name', vendor).execute()
                ...
                if existing.data and len(existing.data) > 0:
                    ...
                    self.supabase.table('vendor_products').update(update_data).eq('vendor_name', vendor).execute()
                else:
                    # Insert new
                    insert_data = {
                        'vendor_name': vendor,
    ...
    def _import_category_patterns(self):
        ...
        for pattern_key, data in self.category_patterns.items():
            ...
            try:
                ...
                existing = self.supabase.table('vendor_product_patterns').select('*').eq('pattern_type', category_type).eq('pattern_value', category_value).execute()
                ...
                if existing.data and len(existing.data) > 0:
                    ...
                    }).eq('pattern_type', category_type).eq('pattern_value', category_value).execute()
                else:
                    # Insert new
    ...
    def _import_citation_patterns(self):
        ...
        for refund_basis, data in self.citation_patterns.items():
            ...
            try:
                ...
                existing = self.supabase.table('refund_citations').select('*').eq('refund_basis', refund_basis).execute()
    ...
    def _import_keyword_patterns(self):
        ...
        for keyword_signature, data in self.keyword_patterns.items():
            ...
            try:
                ...
                # Check if pattern already exists
                existing = self.supabase.table('keyword_patterns').select('*').eq('keyword_signature', signature_str).execute()
                ...
                logger.error(f"Error importing keyword pattern {keyword_signature[:3]}: {e}")
    ...
    def _print_dry_run_summary(self):
        ...
        for vendor, data in sorted(self.vendor_patterns.items(), key=lambda x: x[1]['refund_count'], reverse=True)[:10]:
            ...
            print(f"  {vendor[:40]:40} {success_rate:5.0%} ({data['refund_count']}/{total}) - {product}")

        print(f"\nCategory Patterns ({len(self.category_patterns)}):")
        for pattern_key, data in sorted(self.category_patterns.items(), key=lambda x: x[1]['refund_count'], reverse=True)[:10]:
            total = data['refund_count'] + data['no_opp_count']
            success_rate = data['refund_count'] / total if total > 0 else 0
            category_type, category_value = pattern_key.split('::', 1)
            print(f"  {category_value[:40]:40} {success_rate:5.0%} ({data['refund_count']}/{total}) [{category_type}]")

        print(f"\nCitation Patterns ({len(self.citation_patterns)}):")
        for basis, data in sorted(self.citation_patterns.items(), key=lambda x: x[1]['usage_count'], reverse=True)[:10]:
            print(f"  {basis[:50]:50} {data['usage_count']:5} uses")

        print(f"\nKeyword Patterns ({len(self.keyword_patterns)}):")
        for keyword_sig, data in sorted(self.keyword_patterns.items(), key=lambda x: x[1]['refund_count'], reverse=True)[:10]:
            total = data['refund_count'] + data['no_opp_count']
            success_rate = data['refund_count'] / total if total > 0 else 0
            keywords_str = ', '.join(list(keyword_sig)[:3])  # Show first 3 keywords
            print(f"  [{keywords_str}] {success_rate:5.0%} ({data['refund_count']}/{total})")
...
def main():
    parser = argparse.ArgumentParser(description='Import historical knowledge from Excel files')
    parser.add_argument('--file', required=True, help='Path to Excel file')
    parser.add_argument('--dry-run', action='store_true', help='Dry run - do not import to database')
