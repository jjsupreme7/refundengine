...
def main():
    ...
    parser.add_argument('--output-dir', default='extracted_patterns', help='Output directory')
    ...
    # Extract patterns
    vendor_patterns = defaultdict(lambda: {
        'refund_count': 0,
        'no_opp_count': 0,
    ...
    keyword_patterns = defaultdict(lambda: {
        'refund_count': 0,
        'no_opp_count': 0,
        'refund_bases': Counter(),
        'keywords': set(),
    ...
    citation_patterns = defaultdict(lambda: {
        'usage_count': 0,
        'example_cases': [],
    })
    ...
    for idx, row in analyzed_df.iterrows():
        ...
        if description_col:
            ...
            if pd.notna(description) and description != '':
                ...
                if len(keywords) >= 2:
                    ...
                    if refund_basis_col and pd.notna(row.get(refund_basis_col)) and is_refund:
                        refund_basis = str(row.get(refund_basis_col)).strip()
                        keyword_patterns[keyword_signature]['refund_bases'][refund_basis] += 1
        ...
        if refund_basis_col:
            ...
            if pd.notna(refund_basis) and refund_basis != '':
                ...
                if len(citation_patterns[refund_basis]['example_cases']) < 3 and vendor_col:
    ...
    for vendor, data in vendor_patterns.items():
        ...
        vendor_keywords = list(data['vendor_keywords']) if data['vendor_keywords'] else []
    ...
    for vendor in sorted(vendor_data, key=lambda x: (x['historical_success_rate'], x['historical_sample_count']), reverse=True)[:10]:
        print(f"  {vendor['vendor_name'][:50]:50} {vendor['historical_success_rate']:.0%} ({vendor['historical_sample_count']} cases)")
