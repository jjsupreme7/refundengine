...
class CorrectionImporter:
    ...
    def get_analysis_id(self, row_id: int) -> Optional[str]:
        ...
        try:
            response = supabase.table('analysis_results').select('id').eq(
                'row_id', row_id
    ...
    def import_review(self, row: pd.Series) -> bool:
        ...
        if review_status == 'corrected' or review_status == 'needs correction':
            ...
            if pd.notna(row.get('Corrected_Estimated_Refund')):
                ...
                corrections['corrected_estimated_refund'] = float(row['Corrected_Estimated_Refund'])
                # Calculate percentage
                if row['Tax'] > 0:
                    corrections['corrected_refund_percentage'] = (
                        float(row['Corrected_Estimated_Refund']) / float(row['Tax']) * 100
                    )
        ...
        review_data = {
            ...
            'review_status': 'approved' if review_status == 'approved' else (
                'corrected' if review_status in ['corrected', 'needs correction'] else 'rejected'
            ),
    ...
    def learn_from_correction(self, row: pd.Series, review_id: str, corrections: Dict):
        ...
        if 'corrected_product_type' in corrections:
            product_desc = corrections.get('corrected_product_desc', row.get('AI_Product_Desc'))
            ...
            try:
                ...
                # Check if exists
                existing = supabase.table('vendor_products').select('id').eq(
                    'vendor_name', vendor_name
                ).eq('product_description', product_desc).execute()
                ...
                if existing.data:
                    ...
                    supabase.table('vendor_products').update({
                        'product_type': product_type,
                        'last_seen_date': datetime.utcnow().isoformat(),
                        'frequency': supabase.table('vendor_products').select('frequency').eq(
                            'id', existing.data[0]['id']
                        ).execute().data[0]['frequency'] + 1
            ...
            try:
                ...
                existing = supabase.table('vendor_product_patterns').select('id').eq(
                    'vendor_name', vendor_name
                ).eq('product_keyword', product_keyword).execute()

                if existing.data:
                    # Update times_confirmed
                    current = supabase.table('vendor_product_patterns').select('times_confirmed').eq(
                        'id', existing.data[0]['id']
                    ).execute()
                    ...
                    # Insert new pattern
                    supabase.table('vendor_product_patterns').insert(pattern_data).execute()
        ...
        try:
            for field, new_value in corrections.items():
                ...
                audit_data = {
                    ...
                    'change_reason': row.get('Reviewer_Notes') if pd.notna(row.get('Reviewer_Notes')) else None,
                    'row_id': int(row['Row_ID']),
                    'vendor_name': row['Vendor']
                }
...
def main():
    parser = argparse.ArgumentParser(description='Import corrections from reviewed Excel')
    parser.add_argument('reviewed_excel', help='Path to reviewed Excel file with corrections')
