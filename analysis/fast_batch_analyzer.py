...
def categorize_product(description: str, vendor_info: Optional[Dict] = None) -> str:
    ...
    # Check keywords
    if any(kw in desc_lower for kw in ['saas', 'subscription', 'cloud', '365', 'workspace']):
        return 'saas_subscription'
    elif any(kw in desc_lower for kw in ['consulting', 'professional services', 'advisory', 'hours']):
        return 'professional_services'
    elif any(kw in desc_lower for kw in ['ec2', 'vm', 'virtual machine', 'infrastructure', 'iaas']):
        return 'iaas_paas'
...
def search_legal_docs(category: str, state: str = 'WA') -> List[Dict]:
    ...
    try:
        ...
        results = supabase.rpc('match_documents_by_state', {
...
def analyze_batch(items: List[Dict], legal_context: Dict[str, List], state: str = 'WA') -> List[Dict]:
    """
    ...
    for i, item in enumerate(items, 1):
        ...
        if vendor_info:
            items_text += f"   Vendor Type: {vendor_info.get('business_model', 'Unknown')}\n"
  ...
  "analyses": [
    ...
    try:
        response = openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a Washington State tax expert. Provide accurate analysis with legal citations."},
...
def main():
    ...
    parser.add_argument("--state", default="washington", help="State to analyze for (default: washington)")
    ...
    for idx, row in df.iterrows():
        ...
        if matched_item:
            ...
            category = categorize_product(matched_item.get('description', ''), vendor_info)
    ...
    for i, analysis in enumerate(all_analyses):
        if i < len(analysis_queue):
            ...
            df.loc[idx, 'Estimated_Refund'] = f"${analysis.get('estimated_refund_amount', 0):,.2f}"
