...
class InvoicePatternLearner:
    ...
    def train_from_excel(self, excel_path: str, file_type: str = 'auto'):
        ...
        """
        from analysis.excel_processors import auto_detect_file_type, DenodoSalesTaxProcessor, UseTaxProcessor
    ...
    def _train_from_denodo(self, df: pd.DataFrame):
        ...
        for idx, row in df.iterrows():
            ...
            # Learn from decision
            is_refund = 'REFUND' in final_decision or 'OPP' in final_decision and 'NO' not in final_decision
    ...
    def classify_invoice(self, invoice_data: Dict) -> Dict:
        ...
        vertex_cat = invoice_data.get('Vertex Category') or invoice_data.get('vertex_category')
        if vertex_cat and vertex_cat in self.patterns['vertex_categories']:
            ...
            refund_count = sum(count for decision, count in distribution.items() if 'REFUND' in decision)
            no_opp_count = sum(count for decision, count in distribution.items() if 'NO OPP' in decision)
            ...
            if total > 0:
                ...
                if refund_pct > 0.7:
                    score += 3.0
                    reasons.append(f"Vertex category '{vertex_cat}' shows {refund_pct:.0%} refund rate")
                    matched_patterns['vertex_category'] = (vertex_cat, refund_pct)
                elif no_opp_pct > 0.7:
                    score -= 2.0
                    reasons.append(f"Vertex category '{vertex_cat}' shows {no_opp_pct:.0%} no-opp rate")

        # Check material group
        material_group = invoice_data.get('matk1_po_material_group_desc') or invoice_data.get('Material Group')
        if material_group and material_group in self.patterns['material_groups']:
            ...
            if total > 5:  # Only trust if we have enough samples
                ...
                if refund_pct > 0.6:
                    score += 1.5
                    reasons.append(f"Material group '{material_group}' shows {refund_pct:.0%} refund rate")
                    matched_patterns['material_group'] = (material_group, refund_pct)
                elif refund_pct < 0.2:
                    score -= 1.0
                    reasons.append(f"Material group '{material_group}' rarely refunds ({refund_pct:.0%})")

        # Check vendor history
        vendor = invoice_data.get('Vendor Name') or invoice_data.get('vendor') or invoice_data.get('name1_po_vendor_name')
        if vendor and vendor in self.patterns['vendor_patterns']:
            ...
            if total > 3:
                ...
                if refund_pct > 0.5:
                    score += 1.0
                    reasons.append(f"Vendor '{vendor}' has {refund_pct:.0%} refund rate ({total} samples)")
                    matched_patterns['vendor'] = (vendor, refund_pct)

        # Analyze description for keywords
        description = str(invoice_data.get('Description') or invoice_data.get('txz01_po_description') or '')
        if description:
            desc_keywords = self._extract_keywords(description.lower())

            refund_keyword_score = sum(self.patterns['refund_keywords'].get(kw, 0) for kw in desc_keywords)
            no_opp_keyword_score = sum(self.patterns['no_opp_keywords'].get(kw, 0) for kw in desc_keywords)

            if refund_keyword_score > no_opp_keyword_score and refund_keyword_score > 5:
                score += 0.5
                top_kws = [kw for kw in desc_keywords if self.patterns['refund_keywords'].get(kw, 0) > 2]
                if top_kws:
                    reasons.append(f"Description contains refund keywords: {', '.join(top_kws[:3])}")
            elif no_opp_keyword_score > refund_keyword_score and no_opp_keyword_score > 5:
                score -= 0.5
    ...
    def get_top_refund_indicators(self, top_n: int = 20) -> Dict:
        ...
        top_no_opp_kw = sorted(
            self.patterns['no_opp_keywords'].items(),
            key=lambda x: x[1],
            reverse=True
        )[:top_n]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Example: python invoice_pattern_learning.py 'Test Data/Phase_3_2023_Use Tax_10-17-25.xlsx'")
