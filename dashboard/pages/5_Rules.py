...
# Page configuration
st.set_page_config(
    page_title="Rules & Guidance - TaxDesk",
    page_icon="📚",
    layout="wide"
)
...
with col3:
    filter_jurisdiction = st.selectbox(
        "Jurisdiction",
        ["All", "Washington", "Oregon", "California"]
    )
...
for category, category_rules in sorted(categories.items()):
    with st.expander(f"📂 {category} ({len(category_rules)})", expanded=True):
        for rule in category_rules:
            ...
            with col1:
                if st.button("👁️ View Full Text", key=f"view_{rule['id']}", use_container_width=True):
                    st.info(f"Viewing {rule['title']}")

            with col2:
                if st.button("📚 Related Docs", key=f"related_{rule['id']}", use_container_width=True):
                    st.info("Related documents coming soon")

            with col3:
                if st.button("🔗 Copy Citation", key=f"copy_{rule['id']}", use_container_width=True):
