...
st.markdown('<div class="main-subtitle">Performance metrics, AI accuracy, and refund analysis</div>', unsafe_allow_html=True)
...
def get_analysis_overview():
    ...
    try:
        ...
        refund_result = supabase.table('analysis_results').select('ai_estimated_refund').execute()
        total_refund = sum([r.get('ai_estimated_refund', 0) or 0 for r in refund_result.data]) if refund_result.data else 0

        # Items pending review
        pending_result = supabase.table('analysis_results') \
...
def get_correction_stats():
    ...
    try:
        ...
        reviews = supabase.table('analysis_reviews').select('fields_corrected, review_status').execute()
...
def get_refund_breakdown():
    ...
    try:
        ...
        df = pd.DataFrame(results.data)
        df['ai_estimated_refund'] = pd.to_numeric(df['ai_estimated_refund'], errors='coerce').fillna(0)

        # Group by exemption type
        breakdown = df.groupby('ai_exemption_type')['ai_estimated_refund'].sum().sort_values(ascending=False)
...
def get_vendor_opportunities():
    ...
    try:
        ...
        vendor_totals = df.groupby('vendor_name')['ai_estimated_refund'].sum() \
...
def get_learning_progress():
    ...
    try:
        ...
        patterns_result = supabase.table('vendor_product_patterns').select('id', count='exact').execute()
        patterns_count = patterns_result.count if hasattr(patterns_result, 'count') else 0

        # Product types cataloged
        products_result = supabase.table('vendor_products').select('id', count='exact').execute()
        products_count = products_result.count if hasattr(products_result, 'count') else 0
...
if ai_perf is not None and len(ai_perf) > 0:
    ...
    with col1:
        ...
        fig_conf = px.histogram(
            ...
            labels={'ai_confidence': 'Confidence Score (%)', 'count': 'Number of Items'},
    ...
    with col2:
        ...
        st.metric("Low Confidence Items", f"{low_conf_pct:.1f}%", help="Items below 70% confidence")
    ...
    if corrections is not None and len(corrections) > 0:
        ...
        st.markdown("*These fields are most often corrected by reviewers - areas where AI needs improvement*")
    ...
    st.info("📊 No analysis data available yet. Run invoice analysis to see AI performance metrics.")
...
if refund_breakdown is not None and len(refund_breakdown) > 0:
    ...
    with col2:
        ...
        breakdown_df['Total Refund'] = breakdown_df['Total Refund'].apply(lambda x: f"${x:,.2f}")
...
st.markdown("---")
st.caption(f"🔄 Last refreshed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Data cached for 5 minutes")
