...
from dashboard.utils.data_loader import load_analyzed_transactions, get_projects_from_db

# Page configuration
st.set_page_config(
    page_title="Claims - TaxDesk",
    page_icon="📋",
    layout="wide"
)
...
approved_df = df[df['Final_Decision'].str.contains('Add to Claim', na=False)] if not df.empty else pd.DataFrame()
...
if not claims:
    ...
    for claim in claims:
        ...
        with col1:
            if st.button("👁️ View Details", key=f"view_{claim['id']}", use_container_width=True):
    ...
    with tab2:
        ...
        if not approved_df.empty:
            display_df = approved_df[[
                ...
                'Total_Amount', 'Tax_Amount', 'Estimated_Refund', 'AI_Confidence'
            ]].copy()
            ...
            display_df['Estimated_Refund'] = display_df['Estimated_Refund'].apply(lambda x: f"${x:,.2f}")
            display_df['AI_Confidence'] = display_df['AI_Confidence'].apply(lambda x: f"{x:.1f}%")
