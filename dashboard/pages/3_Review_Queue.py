...
st.markdown(
    '<div class="main-subtitle">Review transactions flagged for manual analysis</div>',
...
if filtered_df.empty:
    st.success(
        "🎉 No transactions in review queue! All items meet confidence threshold."
    )
else:
    # Bulk actions
        ...
        with st.expander(
            ...
            with col2:
                ...
                analyst_decision = st.radio(
                    ...
                    [
                        "Pending Review",
                        "Approve (Add to Claim)",
                        "Reject (Not Refundable)",
                        "Need More Info",
                    ],
                    key=f"decision_{idx}",
                )
            ...
            if pd.notna(row.get("Legal_Citation")):
                ...
                with col_legal2:
                    if st.button(
                        ...
                        key=f"search_law_{idx}",
                        use_container_width=True,
                    ):
                        st.info(
                            "Search RAG system for related rules (feature coming soon)"
                        )
            else:
                st.info(
                    "No specific legal citation provided. Consider researching applicable tax law for this transaction type."
                )
