...
st.markdown(
    '<div class="main-subtitle">Upload and manage invoices, contracts, and supporting documents</div>',
...
if st.session_state.get("show_upload", False):
    with st.expander("📤 Upload New Documents", expanded=True):
        ...
        if uploaded_files:
            ...
            for file in uploaded_files:
                ...
                if file_ext in DANGEROUS_EXTENSIONS:
                    validation_errors.append(
                        f"❌ {file.name}: Dangerous file type not allowed ({file_ext})"
                    )
                    continue

                if file_ext not in ALLOWED_EXTENSIONS:
                    validation_errors.append(
                        f"❌ {file.name}: File type not allowed ({file_ext})"
                    )
                    continue
        ...
        notes = st.text_area(
            "Notes (Optional)",
            placeholder="Any additional context about these documents...",
        )
        ...
        with col1:
            if st.button(
                ...
                if valid_files:
                    st.success(
                        f"✅ Uploaded {len(valid_files)} documents successfully!"
                    )
                    st.info(
                        "Documents are being processed. Check back soon for analysis results."
                    )
...
with col3:
    projects = get_projects_from_db()
    filter_project = st.selectbox("Project", ["All"] + [p["name"] for p in projects])
...
if not documents:
    ...
    for doc in documents:
        ...
        with col2:
            if st.button(
                "🔄 Reprocess", key=f"reprocess_{doc['id']}", use_container_width=True
            ):
                st.success(f"Reprocessing {doc['id']}")

        with col3:
            if st.button(
                "🗑️ Delete", key=f"delete_{doc['id']}", use_container_width=True
            ):
        ...
        if st.session_state.get(view_key, False):
            with st.expander(f"📄 Document Details: {doc['id']}", expanded=True):
                ...
                if file_path.exists():
                    if file_ext == "PDF":
                        ...
                        try:
                            ...
                            st.info(
                                "💡 This appears to be a placeholder file. In production, actual scanned invoice images would be stored here."
                            )
                    ...
                    st.info(
                        "The file may not have been uploaded yet or the path is incorrect."
                    )

                # Purchase Order Preview Section
                if doc.get("purchase_order_file") and pd.notna(
                    doc["purchase_order_file"]
                ):
                    st.markdown("---")
                    st.markdown("#### 📋 Related Purchase Order")

                    po_file_path = (
                        Path("test_data/purchase_orders") / doc["purchase_order_file"]
                    )

                    if po_file_path.exists():
                        st.markdown(
                            f"**PO Number:** {doc.get('purchase_order', 'N/A')}"
                        )
                ...
                if st.button(
                    "❌ Close", key=f"close_{doc['id']}", use_container_width=False
                ):
                    st.session_state[view_key] = False
                    st.rerun()
