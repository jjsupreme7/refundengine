...
def generate_answer(
    question: str, search_result: Dict, use_learning: bool = True
) -> str:
...
"""
    ...
    if use_learning:
        ...
        golden_examples = st.session_state.feedback_system.get_golden_examples(
            question, limit=2
        )
...
- Be thorough but organized - our clients expect detailed technical analysis"""
    ...
    if golden_examples:
        for example in golden_examples:
            messages.append(
                {"role": "user", "content": f"Example question: {example['question']}"}
            )
            messages.append({"role": "assistant", "content": example["golden_answer"]})
...
def render_feedback_widget(message_index: int):
    ...
    with col1:
        if st.button(
            "👍 Helpful", key=f"thumbs_up_{message_index}", use_container_width=True
        ):
    ...
    if st.session_state.get(f"show_detailed_feedback_{message_index}", False):
        with st.expander("✍️ Provide Detailed Feedback", expanded=True):
            ...
            available_citations = [
                c.get("citation") for c in retrieved_chunks if c.get("citation")
            ]
            ...
            if st.button(
                "Submit Detailed Feedback", key=f"submit_feedback_{message_index}"
            ):
                feedback_data = {
                    "feedback_type": {
                    ...
                    "suggested_structure": (
                        suggested_structure if suggested_structure else None
                    ),
                    "suggested_citations": (
                        suggested_citations if suggested_citations else None
                    ),
                    "feedback_comment": feedback_comment if feedback_comment else None,
                }

                save_feedback(
                    message_index, feedback_data["feedback_type"], **feedback_data
                )
                st.success(
                    "✅ Detailed feedback saved! The system will learn from this."
                )
...
def save_feedback(message_index: int, feedback_type: str, **kwargs):
    ...
    feedback_data = {"feedback_type": feedback_type, **kwargs}
...
def render_learning_insights():
    ...
    try:
        improvements = (
            st.session_state.feedback_system.supabase.table("learned_improvements")
            .select("*")
            .eq("is_active", True)
        ...
        st.info(
            "Learning insights will appear here as the system learns from feedback."
        )
        st.markdown("</div>", unsafe_allow_html=True)
        return
    ...
    if improvements.data:
        ...
        for imp in improvements.data[:3]:
            ...
            st.markdown(
                f"- {imp['improvement_type']}: {validation_rate:.0%} success rate"
            )
    else:
        st.markdown(
            "_No improvements learned yet. Provide feedback to help the system learn!_"
        )

    # Get golden dataset count
    golden = (
        st.session_state.feedback_system.supabase.table("golden_qa_pairs")
...
def get_file_url_for_document(document_id: str) -> tuple:
    ...
    try:
        ...
        result = (
            ...
            .select("file_url, source_file")
            .eq("id", document_id)
            .execute()
        )
        if result.data and len(result.data) > 0:
            return (
                result.data[0].get("file_url", ""),
                result.data[0].get("source_file", ""),
            )
    except Exception as e:
...
def main():
    # Header
    st.markdown(
        '<div class="main-header">🧠 Enhanced RAG with Continuous Learning</div>',
        unsafe_allow_html=True,
    )
    ...
    if prompt := st.chat_input("Ask a question about Washington tax law..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("🧠 Making intelligent decision..."):
                ...
                if law_version in ["old_law", "new_law"] and search_result.get(
                    "results"
                ):
                    search_result["results"] = (
                        st.session_state.law_version_handler.filter_by_law_version(
                            search_result["results"], law_version
                        )
                    )
                    ...
                    old_law_results = (
                        st.session_state.law_version_handler.filter_by_law_version(
                            search_result["results"], "old_law"
                        )
                    )
                    new_law_results = (
                        st.session_state.law_version_handler.filter_by_law_version(
                            search_result["results"], "new_law"
                        )
                    )
                ...
                if not search_result.get("results"):
                    ...
                    with st.spinner("✍️ Generating answer..."):
                        answer = generate_answer(
                            prompt, search_result, use_learning=use_learning
                        )
                    ...
                    if search_result.get("comparison_mode"):
                        ...
                        sources_msg["old_law_results"] = search_result.get(
                            "old_law_results", []
                        )
                        sources_msg["new_law_results"] = search_result.get(
                            "new_law_results", []
                        )
        ...
        if search_result.get("comparison_mode"):
            ...
            with st.expander("📚 View Sources - Comparison Mode", expanded=True):
                ...
                if old_law_sources and new_law_sources:
                    ...
                    comparison = (
                        st.session_state.law_version_handler.compare_old_vs_new(
                            prompt, old_law_sources, new_law_sources
                        )
                    )
    ...
    st.divider()
    st.caption(
        "🧠 Powered by Enhanced RAG + Continuous Learning | 📚 Washington State Tax Law"
    )
