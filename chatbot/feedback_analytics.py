...
def get_improvements_data():
    ...
    result = supabase.table("learned_improvements")\
        .select("*")\
        .order("created_at", desc=True)\
        .execute()
...
def get_golden_data():
    ...
    result = supabase.table("golden_qa_pairs")\
        .select("*")\
        .order("created_at", desc=True)\
        .execute()
...
def get_citation_preferences():
    ...
    result = supabase.table("citation_preferences")\
        .select("*")\
...
def main():
    ...
    with col3:
        ...
        active_improvements = len(improvements_df[improvements_df['is_active'] == True]) if not improvements_df.empty else 0
        st.metric("Active Improvements", f"{active_improvements}/{total_improvements}")

    with col4:
        total_golden = len(golden_df)
        verified_golden = len(golden_df[golden_df['is_verified'] == True]) if not golden_df.empty else 0
    ...
    if not feedback_df.empty and 'created_at' in feedback_df.columns:
        ...
        feedback_by_date = feedback_df.groupby(['date', 'feedback_type']).size().reset_index(name='count')
    ...
    if not improvements_df.empty:
        ...
        if not active_improvements_df.empty:
            ...
            improvement_type_counts = active_improvements_df['improvement_type'].value_counts()
            ...
            top_improvements = active_improvements_df.nlargest(10, 'validation_rate')[
                ['improvement_type', 'confidence', 'times_applied', 'times_validated', 'validation_rate']
            ]
    ...
    if not citation_prefs_df.empty:
        ...
        top_citations = citation_prefs_df.nlargest(20, 'preference_score')[
            ['citation', 'preference_score', 'times_suggested_by_user',
             'times_retrieved_and_liked', 'times_retrieved_and_disliked']
        ]

        st.dataframe(
            top_citations.style.format({
                'preference_score': '{:.2f}'
            }),
            use_container_width=True
        )
    ...
    if not golden_df.empty:
        ...
        if 'created_at' in golden_df.columns:
            recent_golden = golden_df.nlargest(10, 'created_at')[
                ['question', 'query_category', 'difficulty', 'is_verified', 'created_at']
            ]
            st.dataframe(recent_golden, use_container_width=True)
    else:
        st.info("No golden Q&A pairs yet. High-rated responses will be added automatically!")
    ...
    if not feedback_df.empty:
        ...
        if not detailed_feedback.empty:
            for idx, row in detailed_feedback.iterrows():
                with st.expander(f"{row['feedback_type']} - {row['created_at']} - Rating: {row.get('rating', 'N/A')}"):
