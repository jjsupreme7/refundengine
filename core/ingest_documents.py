...
def extract_text_from_pdf(pdf_path: str, max_pages: int = 3) -> tuple[str, int]:
    ...
    Note: For more comprehensive extraction including TIF, MSG, and other formats,
    use core.document_extractors.extract_text_from_file() instead.
    """
    try:
...
def suggest_metadata_simple(
    pdf_path: str, total_pages: int, document_type: str
) -> Dict[str, Any]:
    """
    ...
    if document_type == "tax_law":
        ...
        citation_match = re.search(
            r"((?:WAC|RCW)\s*[\d\-\.]+)", filename, re.IGNORECASE
        )
        metadata.update(
            {
...
def suggest_metadata_with_ai(
    pdf_path: str, sample_text: str, document_type: str, total_pages: int = 0
) -> Dict[str, Any]:
    """
    ...
    try:
        response = openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"You are an expert at analyzing {'tax law' if document_type == 'tax_law' else 'vendor/product'} documents. Extract metadata accurately.",
                },
...
def export_metadata_to_excel(
    ...
    print(
        "This will NOT ingest to database yet - just export to Excel for your review\n"
    )
    ...
    for pdf_path in tqdm(pdf_files, desc="Analyzing files"):
        try:
            ...
            if total_pages >= 100:
                ...
                suggested_metadata = suggest_metadata_with_ai(
                    str(pdf_path), sample_text, document_type, total_pages
                )
                ...
                row.update(
                    {
                        ...
                        "topic_tags": ", ".join(
                            suggested_metadata.get("topic_tags", [])
                        ),
                        "tax_types": ", ".join(suggested_metadata.get("tax_types", [])),
                        "industries": ", ".join(
                            suggested_metadata.get("industries", [])
                        ),
                        "keywords": ", ".join(suggested_metadata.get("keywords", [])),
                        "referenced_statutes": ", ".join(
                            suggested_metadata.get("referenced_statutes", [])
                        ),
                    }
                )
            else:  # vendor
                row.update(
                    {
                        ...
                        "vendor_category": suggested_metadata.get(
                            "vendor_category", ""
                        ),
                        "document_category": suggested_metadata.get(
                            "document_category", ""
                        ),
                        "industry": suggested_metadata.get("industry", ""),
                        "business_model": suggested_metadata.get("business_model", ""),
                        "primary_products": ", ".join(
                            suggested_metadata.get("primary_products", [])
                        ),
                        "typical_delivery": suggested_metadata.get(
                            "typical_delivery", ""
                        ),
                        "tax_notes": suggested_metadata.get("tax_notes", ""),
                        "product_types": ", ".join(
                            suggested_metadata.get("product_types", [])
                        ),
                        "industries": ", ".join(
                            suggested_metadata.get("industries", [])
                        ),
                        "keywords": ", ".join(suggested_metadata.get("keywords", [])),
                        "confidence_score": suggested_metadata.get(
    ...
    for idx, row in approved_df.iterrows():
        try:
            ...
            is_manual_entry = (
                ...
                or str(pdf_path) == "N/A"
                or str(pdf_path).endswith(".manual")
            )
                ...
                doc_data.update(
                    {
                        ...
                        "referenced_statutes": parse_array(
                            row.get("referenced_statutes", "")
                        ),
                    }
                )
            else:  # vendor
                ...
                doc_data.update(
                    {
                        ...
                        "business_model": (
                            ...
                            if pd.notna(row.get("business_model"))
                            else None
                        ),
                        "primary_products": parse_array(
                            row.get("primary_products", "")
                        ),
                        "typical_delivery": (
                            row.get("typical_delivery")
                            if pd.notna(row.get("typical_delivery"))
                            else None
                        ),
                        "tax_notes": (
                            row.get("tax_notes")
                            if pd.notna(row.get("tax_notes"))
                            else None
                        ),
                        "confidence_score": (
                            float(row.get("confidence_score", 80.0))
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified document ingestion for tax law and vendor documents"
    )
    parser.add_argument(
        "--type",
    ...
    parser.add_argument(
        "--export-metadata", help="Export AI metadata to Excel (no ingestion)"
    )
    parser.add_argument(
        "--import-metadata", help="Import edited Excel and ingest to Supabase"
    )
    parser.add_argument(
