...
class ExcelVersionManager:
    ...
    def upload_file(
        ...
        result = self.supabase.table('excel_file_tracking').insert({
    ...
    def create_version(
        ...
        file_record = self.supabase.table('excel_file_tracking').select('*').eq('id', file_id).single().execute()
    ...
    def acquire_lock(self, file_id: str, user_email: str) -> bool:
        ...
        """
        result = self.supabase.rpc('acquire_file_lock', {
    ...
    def release_lock(self, file_id: str, user_email: str) -> bool:
        ...
        """
        result = self.supabase.rpc('release_file_lock', {
            'file_id_param': file_id,
            'user_email_param': user_email
        }).execute()
    ...
    def get_version_history(self, file_id: str) -> List[Dict]:
        ...
        """
        result = self.supabase.table('excel_file_versions').select('*').eq('file_id', file_id).order('version_number', desc=True).execute()

        return result.data

    def download_version(self, file_id: str, version_number: int, output_path: str) -> str:
        ...
        """
        # Get version record
        version_record = self.supabase.table('excel_file_versions').select('*').eq('file_id', file_id).eq('version_number', version_number).single().execute()
        ...
        file_bytes = self.supabase.storage.from_('excel-versions').download(storage_path)
...
def create_new_version(file_id: str, file_path: str, user_email: str, change_summary: str) -> str:
